...
print "ssh -t -i $pem $user\@$esp \"bash configureHPCC.sh $private_ips $created_environment_file $supportnodes $non_support_instances $roxienodes $slavesPerNode\"\n";
system("ssh -t -i $pem $user\@$esp \"bash configureHPCC.sh $private_ips $created_environment_file $supportnodes $non_support_instances $roxienodes $slavesPerNode\"");
